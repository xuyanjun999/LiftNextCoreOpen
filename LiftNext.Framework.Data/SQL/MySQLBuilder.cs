...
namespace LiftNext.Framework.Data.SQL
{
    public class MySQLBuilder : ISQLBuilder
    {
        ...
        public StringBuilder BuildDeleteSql<T>(string tableName, IEnumerable<T> entitys) where T : class
        {
            ...
            int[] ids = entitys.Select(x => (int)idPropertyInfo.GetValue(x)).ToArray();
        ...
        /// <param name="id"></param>
        /// <returns></returns>
        public StringBuilder BuildDeleteSql(string tableName, int id)
        {
            StringBuilder sql = new StringBuilder();
        ...
        #region 构造删除语句
        public StringBuilder BuildDeleteSql(string tableName, int[] ids)
        {
            StringBuilder sql = new StringBuilder();
        ...
        PropertyInfo[] GetNeedHandleProperty(object entity)
        {
            ...
            foreach (var propertyInfo in propertyInfos)
            {
                ...
                //过滤掉 实体
                if (propertyInfo.PropertyType.FullName.Contains("LiftNext.Framework")&&!propertyInfo.PropertyType.IsEnum) continue;
